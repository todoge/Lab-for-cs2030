        ...
        for (int i = 0; i < numOfCustomers; i++) {
                    ...
                    while (waitingQueue.size() != 0 && !waiter.isBusy()) {
                        ...
                        if (waiter.servingUntil() <= arrivalTimes.get(i)) {
                            events.add(new Event(
                                    States.DONE, waiter.getServing(), waiter.servingUntil()
                            ));
                            waiter.free();
                        }
                    }
            ...
            if (waiter.isBusy()) {
                ...
                if (waitingQueue.size() >= maxWaitingCustomers) {
                    ...
                    events.add(new Event(States.WAITS,newCustomer,arrivalTimes.get(i)));
                ...
                totalCustomersServed++;
                events.add(new Event(States.SERVED, newCustomer, arrivalTimes.get(i)));
            }
        }
