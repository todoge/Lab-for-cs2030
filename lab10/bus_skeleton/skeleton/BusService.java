...
class BusService {
  ...
  public CompletableFuture<Set<BusStop>> getBusStops() {
    CompletableFuture<Scanner> sc = BusAPI.getBusStopsServedBy(serviceId).thenApply(service ->
            new Scanner(service));
    CompletableFuture<Set<BusStop>> stops = sc.thenApply(scanner -> scanner.useDelimiter("\n")
        .tokens()
        .map(line -> line.split(","))
