...
class BusRoutes {
  ...
  public CompletableFuture<String> description() {
    CompletableFuture<String> result = CompletableFuture.completedFuture( "Search for: " + this.stop + " <-> " + name + ":\n"
    + "From " +  this.stop + "\n");

    for (BusService service: services.keySet()) {
      CompletableFuture<Set<BusStop>> stops = services.get(service);
      result = result.thenCombineAsync(describeService(service,stops), (x,y)->x+y);
    }
    return result;
  }
  ...
  public CompletableFuture<String> describeService(BusService service, CompletableFuture<Set<BusStop>> stops) {
    ...
    return stops.thenApply(busStops -> {
      if (busStops.isEmpty()) {
        return "";
      }
